...
function findLowestAverages(students)
{
	...
	// min_grade[g] stores the lowest recorded grade for a student in the grade g
	min_grades[6] = 100, min_grades[7] = 100, min_grades[8] = 100;
	...
	for (var i = 0; i < students.length; i++) {
		...
		for (var s = 0; s < student["scores"].length; s++)
			avg += student["scores"][s]["value"];

		avg /= student["scores"].length;

		// Update min_grades if avg is less than the one in min_grades
		if (avg < min_grades[student["grade"]])
			min_grades[student["grade"]] = avg;
	}
